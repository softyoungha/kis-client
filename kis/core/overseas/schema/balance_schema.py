...
class Stock(BaseModel):
    ...
    frcr_evlu_pfls_amt: float = Field(
        title="외화평가손익금액", description="해당 종목의 매입금액과 평가금액의 외회기준 비교 손익"
    )
...
class PrettyStock(BaseModel):
    """해외주식현재가/해외주식 잔고 - Response Body output1 응답상세1 Pretty"""
    ...
    profit_loss_amount: float = Field(
        alias="frcr_evlu_pfls_amt", title="외화평가손익금액", description="평가손익"
    )
    profit_loss_rate: float = Field(
        alias="evlu_pfls_rt", title="평가손익율", description="수익률"
    )
    inventory_quantity: int = Field(
        alias="ovrs_cblc_qty", title="해외잔고수량", description="보유수량"
    )
    possible_quantity: int = Field(
        alias="ord_psbl_qty", title="주문가능수량", description="매도가능수량"
    )
    avg_purchase_price: float = Field(
        alias="pchs_avg_pric", title="매입평균가격", description="매입단가"
    )
...
class Deposit(BaseModel):
    ...
    purchase_amount: float = Field(alias="frcr_pchs_amt1", title="외화매입금액1")
    realized_profit_loss_amount: float = Field(
        alias="ovrs_rlzt_pfls_amt", title="해외실현손익금액"
    )
    total_realized_profit_loss_amount: float = Field(
        alias="ovrs_rlzt_pfls_amt2", title="해외실현손익금액2"
    )
